...
public class MideaACHandler extends BaseThingHandler implements DiscoveryHandler {
    ...
    private class ConnectionManager {
                            ...
                            if (response.length > 40 + 16) {
                                ...
                                if (data.length > 0) {
                                    if (data.length > 25 || data.length < 21) {
                                        logger.error("Response data is {} long instead of 21!", data.length);
                                        // return;
                                    }
                                    ...
                                    try {
                                        if (bodyType != 30) {
                                            ...
                                            logger.debug("data length is {} version is {} thing is {}", data.length,
        ...
        protected synchronized void disconnect() {
            ...
            writer = null;
            markOffline();
            // dispose(); JO added JO rimmed out
        }
            ...
            try {
                if (inputStream != null) {
                    ...
                    if (len > 0) {
                        ...
                        logger.debug("Thing:{} Response bytes: {}", thing.getUID(), Utils.bytesToHex(bytes));
                        return bytes;
                    }
                }
            } catch (IOException e) {
